...
const PlanItem = ({ icon: Icon, text, time, done }) => (
    <div className={clsx(
        "flex items-center gap-3 p-3 rounded-xl border transition-colors",
        done ? "bg-teal-50 border-teal-100" : "bg-white/10 border-white/20 hover:bg-white/20"
    )}>
        <div className={clsx("w-8 h-8 rounded-full flex items-center justify-center", done ? "text-teal-600 bg-white" : "text-white bg-white/20")}>
            <Icon size={14} />
        </div>
        <div className="flex-1">
            <p className={clsx("text-sm font-bold", done ? "text-teal-800" : "text-white")}>{text}</p>
            <p className={clsx("text-[10px]", done ? "text-teal-600" : "text-teal-100 opacity-80")}>{time}</p>
        </div>
        {done && <div className="w-5 h-5 rounded-full bg-teal-500 flex items-center justify-center text-white"><Award size={10} /></div>}
    </div>
)
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <motion.div className="bg-white rounded-[32px] p-6 shadow-soft border border-slate-100 flex flex-col h-auto sm:h-64">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                        <Calendar size={18} className="text-teal-500" /> Kế hoạch hôm nay
                    </h3>
                    <div className="flex-1 space-y-3">
                        <PlanItem icon={Droplets} text="Uống 2L nước" time="7/8 cốc" done={true} />
                        <PlanItem icon={Footprints} text="Đi bộ 5000 bước" time="5430/5000" done={true} />
                        <PlanItem icon={Moon} text="Ngủ trước 11h" time="Tối nay" done={false} />
                    </div>
                </motion.div>
            </div>
